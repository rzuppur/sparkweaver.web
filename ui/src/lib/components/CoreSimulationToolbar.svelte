<script lang="ts">
  ...
  function toggleAutoRun(): void {
    if (autoRun) {
      ...
      coreService.buildTree($editorTreeString);
  ...
  onMount(() => {
    const treeUnSub = editorTreeString.subscribe(() => {
      if (autoRun) {
...
<div class="project-toolbar">
  {#if $coreReady}
    ...
    <div class="toolbar-label">Simulation</div>
    <div class="toolbar-buttons">
      <!-- svelte-ignore a11y_consider_explicit_label -->
      <button type="button" onclick={toggleAutoRun} title="Autorun">
        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" fill={autoRun ? "#3a6" : "#888"}>
      ...
      <!-- svelte-ignore a11y_consider_explicit_label -->
      <button type="button" onclick={() => coreService.buildTree($editorTreeString)} title="Build">
        <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" fill={$coreSimulationState === CoreSimulationState.ERROR ? "#f52" : "#888"}>
      ...
      {#if $coreSimulationState === CoreSimulationState.PAUSED}
        ...
        <button type="button" onclick={() => coreService.runSimulation()} title="Run">
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" fill="#888">
      ...
      {:else if $coreSimulationState === CoreSimulationState.RUNNING}
        ...
        <button type="button" onclick={() => coreService.stopSimulation()} title="Pause">
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" fill="#fa8">
      ...
      {#if $coreSimulationState === CoreSimulationState.PAUSED}
        ...
        <button type="button" onclick={() => coreService.stepSimulation()} title="Step">
          <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" fill="#aaf">
