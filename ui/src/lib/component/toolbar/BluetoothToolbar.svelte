<script lang="ts">
  ...
  import { bluetoothStore } from "$lib/store/bluetooth.store";
  import { editorStore } from "$lib/store/editor.store";
  import { Uint8Vector } from "$lib/util/vector.util";

  const bluetoothState = bluetoothStore.state;
  const bluetoothPwChange = bluetoothStore.pwChange;

  let newPw = $state("");

  async function changePw(): Promise<void> {
    logBus.writeDebug("BluetoothToolbar: changePw");
    const pw = Number.parseInt(newPw, 10);
    if (pw < 100000 || pw > 999999 || !Number.isInteger(pw)) {
      logBus.writeError("BluetoothToolbar: Password must be 6 digits");
      return;
    }
    const success = await bluetoothService.request(async (service) => {
      const chrPw = await service.getCharacteristic(bluetoothService.CHR_PW);
  ...
  async function loadFromDevice(): Promise<void> {
    logBus.writeDebug("BluetoothToolbar: loadFromDevice");
    if (!(await uiService.confirm("Node labels will be cleared, continue?"))) return;
    try {
      let data: Uint8Array | undefined;
      await bluetoothService.request(async (service) => {
        data = await bluetoothService.readInChunks(service, bluetoothService.CHR_TREE);
      });
      if (data) {
        logBus.writeDebug(`BluetoothToolbar: loadFromDevice ${[...data].map(v => v.toString(16).padStart(2, "0")).join(" ")}`);
        editorService.deserializeFromExternal(new Uint8Vector([...data]), {});
      }
    } catch (e) {
      logBus.writeError(`BluetoothToolbar: ${e}`);
    }
  }

  async function sendToDevice(): Promise<void> {
    logBus.writeDebug("BluetoothToolbar: sendToDevice");
    const project = editorStore.projectValue;
    if (project) await bluetoothService.request(async (service) => {
      await bluetoothService.writeInChunks(service, bluetoothService.CHR_TREE, project.tree.get());
    });
  }
</script>
...
<div class="bluetooth-toolbar">
  ...
  <div class="text">
    ...
    {/if}
    {#if $bluetoothState >= BluetoothState.CONNECTED && $bluetoothPwChange === true}
